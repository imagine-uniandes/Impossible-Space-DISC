...
public class ObjectToggler : MonoBehaviour
{
    ...
    [Tooltip("Puerta que reproducirá la animación antes de desactivarse")]
    public GameObject doorTarget;

    [Tooltip("Nombre del trigger/estado en el Animator de la puerta")]
    public string doorAnimationTrigger = "Open";

    [Tooltip("Sonido de la puerta abriéndose (opcional)")]
    public AudioClip doorSound;

    [Tooltip("Volumen del sonido de la puerta")]
    [Range(0f, 1f)]
    public float doorSoundVolume = 1.0f;
    ...
    private void Start()
    {
        // Crear AudioSource si hay algún sonido asignado
        if (toggleSound != null || doorSound != null)
        {
    ...
    /// <summary>
    /// Ejecuta la acción configurada. Si hay doorTarget, primero reproduce su animación.
    /// </summary>
    private void ExecuteAction()
    {
        if (doorTarget != null)
        {
            StartCoroutine(PlayDoorThenToggle());
        }
        else
        {
            ApplyToggleToTargets();
        }
    }

    private IEnumerator PlayDoorThenToggle()
    {
        Animator doorAnimator = doorTarget.GetComponent<Animator>();
        ...
        if (doorSound != null)
        {
            ...
            audioSource.PlayOneShot(doorSound, doorSoundVolume);
        }

        // Lanzar animación si hay Animator y trigger configurado
        if (doorAnimator != null && !string.IsNullOrEmpty(doorAnimationTrigger))
        {
            doorAnimator.SetTrigger(doorAnimationTrigger);

            if (showLogs)
                Debug.Log($"<color=cyan>[ObjectToggler] 🚪 Animación '{doorAnimationTrigger}' iniciada en '{doorTarget.name}'</color>");

            // Esperar un frame para que el Animator actualice su estado
            yield return null;

            // Esperar a que termine el estado de animación actual
        ...
        else
        {
            if (doorAnimator == null)
                Debug.LogWarning("<color=orange>[ObjectToggler] ⚠️ doorTarget no tiene Animator asignado</color>");
        }

        ApplyToggleToTargets();
    }

    private void ApplyToggleToTargets()
    {
